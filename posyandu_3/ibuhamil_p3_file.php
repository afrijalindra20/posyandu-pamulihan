...
require_once __DIR__ . '/../pos_3/header_ih_p3.php';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action'])) {
        ...
        if ($action == 'import_csv' || $action == 'import_ibu_hamil_csv') {
            ...
            $ibuHamilData = getIbuHamil3AndCatatanKehamilan($db, $id_ibu);
            if ($bulan === 'all') {
                $catatanKehamilanData = getAllCatatan3($db, $id_ibu);
            } else {
                $catatanKehamilanData = getCatatanKehamilanByBulan3($db, $id_ibu, $bulan);
            }
...
$ibuHamils = $db->query("SELECT id_ibu, nama_ibu_hamil FROM ibu_hamil_3")->fetchAll(PDO::FETCH_ASSOC);

// Ambil daftar ibu hamil untuk dropdown
$ibuHamilList = $db->query("SELECT id_ibu, nama_ibu_hamil FROM ibu_hamil_3")->fetchAll(PDO::FETCH_ASSOC);
...
function importFromCSV($filename, $db) {
    ...
    try {
        if (($handle = fopen($filename, "r")) !== FALSE) {
            ...
            $stmt = $db->prepare("INSERT INTO catatan_kehamilan_3 (id_ibu, hamil_keberapa, hpht, hpl, usia_kehamilan, status_kehamilan, tinggi_badan, berat_badan, lila, laboratorium, imunisasi, mendapatkan_bantuan, mempunyai_bpjs, bulan) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
function importIbuHamilFromCSV($filename, $db) {
    ...
    try {
        if (($handle = fopen($filename, "r")) !== FALSE) {
            ...
            $stmt = $db->prepare("INSERT INTO ibu_hamil_3 (id_ibu, nama_ibu_hamil, nik, tempat_tanggal_lahir_ibu, nama_suami, nik_suami, tempat_tanggal_lahir_suami, alamat) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
<?php 
require_once __DIR__ . '/../pos_3/footer_ih_p3.php';
ob_end_flush();
?>