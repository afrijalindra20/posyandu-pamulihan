...
function getBalitaName($db, $id_balita) {
    $stmt = $db->prepare("SELECT nama_balita FROM balita_3 WHERE id_balita = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add' || $action === 'edit') {
       ...
       if ($action === 'add') {
        if (empty($_POST['no'])) {
            $stmt = $db->query("SELECT MAX(no) as max_no FROM pengukuran_balita_3");
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        $stmt->execute(array_values($data));
                ...
                $stmt = $db->prepare("UPDATE pengukuran_balita_3 SET no = ?, id_balita = ?, tanggal_pengukuran = ?, berat_badan = ?, tinggi_badan = ?, status_gizi = ?, bulan = ? WHERE id_pengukuran = ?");
                $stmt->execute(array_values($data));
            }
        } elseif ($action === 'delete') {
            $stmt = $db->prepare("DELETE FROM pengukuran_balita_3 WHERE id_pengukuran = ?");
...
$pengukuran = null;
if (isset($_GET['edit'])) {
    $stmt = $db->prepare("SELECT * FROM pengukuran_balita_3 WHERE id_pengukuran = ?");
...
$pengukuran_balitas = $db->query("SELECT p.*, b.nama_balita FROM pengukuran_balita_3 p JOIN balita_3 b ON p.id_balita = b.id_balita ORDER BY p.no ASC")->fetchAll(PDO::FETCH_ASSOC);
...
<?php require_once __DIR__ . '/../pos_3/footer_balita_p3.php'; ?>