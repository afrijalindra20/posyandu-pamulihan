...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action'])) {
        ...
        if ($bulan === 'all') {
            $pengukuranData = getAllPengukuran($db, $id_balita);
        } else {
...
function importFromCSV($filename, $db) {
    ...
    try {
        if (($handle = fopen($filename, "r")) !== FALSE) {
            ...
            $stmt = $db->prepare("INSERT INTO pengukuran_balita (id_balita, tanggal_pengukuran, berat_badan, tinggi_badan, status_gizi, bulan) VALUES (?, ?, ?, ?, ?, ?)");
...
function importBalitaFromCSV($filename, $db) {
    ...
    try {
        if (($handle = fopen($filename, "r")) !== FALSE) {
            ...
            $stmt = $db->prepare("INSERT INTO balita (id_balita, nama_balita, jenis_kelamin, nik, tanggal_lahir, berat_badan_lahir, nama_ayah, nama_ibu, alamat, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
<?php 
require_once __DIR__ . '/../pos_1/footer_balita_p1.php';
ob_end_flush();
?>