...
function getBalitaName($db, $id_balita) {
    $stmt = $db->prepare("SELECT nama_balita FROM balita_2 WHERE id_balita = ?");
...
if ($action === 'add') {
    ...
    $stmt->execute(array_values($data));
        ...
        $stmt = $db->prepare("DELETE FROM pengukuran_balita_2 WHERE id_pengukuran = ?");
...
$pengukuran = null;
if (isset($_GET['edit'])) {
    $stmt = $db->prepare("SELECT * FROM pengukuran_balita_2 WHERE id_pengukuran = ?");
...
<?php require_once __DIR__ . '/../pos_2/footer_balita_p2.php'; ?>