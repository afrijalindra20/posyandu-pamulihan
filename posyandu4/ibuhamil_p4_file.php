...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action'])) {
        ...
        if ($action == 'import_csv' || $action == 'import_ibu_hamil_csv') {
            ...
            if ($bulan === 'all') {
                $catatanKehamilanData = getAllCatatan4($db, $id_ibu);
            } else {
...
$ibuHamils = $db->query("SELECT id_ibu, nama_ibu_hamil FROM ibu_hamil_4")->fetchAll(PDO::FETCH_ASSOC);
...
function importFromCSV($filename, $db) {
    ...
    try {
        if (($handle = fopen($filename, "r")) !== FALSE) {
            ...
            $stmt = $db->prepare("INSERT INTO catatan_kehamilan_4 (id_ibu, hamil_keberapa, hpht, hpl, usia_kehamilan, status_kehamilan, tinggi_badan, berat_badan, lila, laboratorium, imunisasi, mendapatkan_bantuan, mempunyai_bpjs, bulan) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
function importIbuHamilFromCSV($filename, $db) {
    ...
    try {
        if (($handle = fopen($filename, "r")) !== FALSE) {
            ...
            $stmt = $db->prepare("INSERT INTO ibu_hamil_4 (id_ibu, nama_ibu_hamil, nik, tempat_tanggal_lahir_ibu, nama_suami, nik_suami, tempat_tanggal_lahir_suami, alamat) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
