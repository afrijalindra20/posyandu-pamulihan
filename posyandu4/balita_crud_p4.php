...
function getBalitaName($db, $id_balita) {
    $stmt = $db->prepare("SELECT nama_balita FROM balita_4 WHERE id_balita = ?");
...
$pengukuran = null;
if (isset($_GET['edit'])) {
    $stmt = $db->prepare("SELECT * FROM pengukuran_balita_4 WHERE id_pengukuran = ?");
...
$pengukuran_balitas = $db->query("SELECT p.*, b.nama_balita FROM pengukuran_balita_4 p JOIN balita_4 b ON p.id_balita = b.id_balita ORDER BY p.no ASC")->fetchAll(PDO::FETCH_ASSOC);
...
<?php require_once __DIR__ . '/../pos_4/footer_balita_p4.php'; ?>