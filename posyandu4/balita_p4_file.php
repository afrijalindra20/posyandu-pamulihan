...
require_once __DIR__ . '/../pos_4/header_balita_p4.php';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action'])) {
        ...
        if ($bulan === 'all') {
            $pengukuranData = getAllPengukuran4($db, $id_balita);
        } else {
            $pengukuranData = getPengukuranByBulan4($db, $id_balita, $bulan);
        }
...
$balitas = $db->query("SELECT id_balita, nama_balita FROM balita_4")->fetchAll(PDO::FETCH_ASSOC);
...
function importFromCSV($filename, $db) {
    ...
    try {
        if (($handle = fopen($filename, "r")) !== FALSE) {
            ...
            $stmt = $db->prepare("INSERT INTO pengukuran_balita_4 (id_balita, tanggal_pengukuran, berat_badan, tinggi_badan, status_gizi, bulan) VALUES (?, ?, ?, ?, ?, ?)");
...
function importBalitaFromCSV($filename, $db) {
    ...
    try {
        if (($handle = fopen($filename, "r")) !== FALSE) {
            ...
            $stmt = $db->prepare("INSERT INTO balita_4 (id_balita, nama_balita, jenis_kelamin, nik, tanggal_lahir, berat_badan_lahir, nama_ayah, nama_ibu, alamat, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
<?php 
require_once __DIR__ . '/../pos_4/footer_balita_p4.php';
ob_end_flush();
?>